...
export const yamlRequestToJson = (data: string | any, parsed: boolean = false): any => {
  try {
    ...
    if (params.query && Array.isArray(params.query)) {
      transformedJson.request.params = transformedJson.request.params.concat(params.query.map((param: any) => ({
        ...param,
        type: 'query',
    ...
    if (params.path && Array.isArray(params.path)) {
      transformedJson.request.params = transformedJson.request.params.concat(params.path.map((param: any) => ({
        ...param,
        type: 'path',
        enabled: param.disabled !== true
      })));
